...
simax1 = pl.axes([mainxl,           mainy1+hmain*0.2, wmain*0.4, hmain*0.5])
simax2 = pl.axes([mainxl+wmain*0.6, mainy1+hmain*0.2, wmain*0.4, hmain*0.5])
for ax in [simax1, simax2]:
    ax.set_xticks([], [])
    ax.set_yticks([], [])
    ax.scatter(time, data, c='k', alpha=0.5)
    ax.set_xlim([0,1])
pkw = dict(lw=3, alpha=0.8)
simax1.plot(time, bad_fit,  c=colors[np.argmin(zpts)], **pkw)
simax2.plot(time, good_fit, c=colors[np.argmax(zpts)], **pkw)
